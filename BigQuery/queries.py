...
"""),
    ("Query-11", f"""
        SELECT 
            CAST(JSON_VALUE(orders.json_data, '$.O_ORDERKEY') AS INT64) AS orderkey, 
            CAST(JSON_VALUE(orders.json_data, '$.O_ORDERDATE') AS DATE) AS orderdate, 
            CAST(JSON_VALUE(orders.json_data, '$.O_TOTALPRICE') AS NUMERIC) AS totalprice, 
            lineitems.total_revenue, 
            lineitems.avg_discount
        FROM (
            SELECT 
                CAST(JSON_VALUE(json_data, '$.L_ORDERKEY') AS INT64) AS orderkey, 
                SUM(CAST(JSON_VALUE(json_data, '$.L_EXTENDEDPRICE') AS NUMERIC) * 
                    (1 - CAST(JSON_VALUE(json_data, '$.L_DISCOUNT') AS NUMERIC))) AS total_revenue,
                AVG(CAST(JSON_VALUE(json_data, '$.L_DISCOUNT') AS NUMERIC)) AS avg_discount
            FROM `{full_dataset}.jLINEITEM`
            GROUP BY orderkey
        ) AS lineitems
        JOIN `{full_dataset}.jORDERS` AS orders
            ON lineitems.orderkey = CAST(JSON_VALUE(orders.json_data, '$.O_ORDERKEY') AS INT64)
        ...
        CASE
            WHEN LAG(SUM(quarterly_quantity)) OVER (
                ...
                (SUM(quarterly_quantity) - LAG(SUM(quarterly_quantity)) OVER (
                    PARTITION BY l_suppkey, quarter ORDER BY year
                )) / LAG(SUM(quarterly_quantity)) OVER (
                    PARTITION BY l_suppkey, quarter ORDER BY year
                ) * 100
            ELSE NULL
